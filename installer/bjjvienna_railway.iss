...
OutputDir=..\dist
OutputBaseFilename=BJJVienna-Setup-Railway
Compression=lzma2
...
begin
  ConfigPage := CreateCustomPage(
    wpSelectTasks,
    'Cloud Configuration',
    'Configure Railway API/DB defaults for first run'
  );
  ...
  ApiVerifyTlsCheck.Caption := 'Verify TLS certificates (recommended for Railway HTTPS)';
  ApiVerifyTlsCheck.Checked := True;
end;
